...
from app.tools.repo_tools import REPO_TOOLS, _safe_join
...
def _demo_file_content(path: str, approach: str) -> str:
    """Per-extension plausible stub content for Settings.codegen_demo_mode
    — never invoked in production (codegen_demo_mode defaults False).
    Deliberately generic (no live repo introspection happens in demo
    mode) rather than guessing a structure that hasn't actually been
    inspected."""
    suffix = Path(path).suffix.lower()
    banner = f"Demo execution (Quipu CODEGEN_DEMO_MODE) — {approach}"
    if suffix == ".py":
        return f'"""{banner}"""\n\n\ndef export_report(data):\n    """Placeholder implementation — see the real Codegen path for production runs."""\n    raise NotImplementedError("implemented by real Codegen in production mode")\n'
    if suffix in (".java",):
        return f"// {banner}\npublic class ReportExporter {{\n    // Placeholder implementation — see the real Codegen path for production runs.\n}}\n"
    if suffix == ".feature":
        return f"# {banner}\nFeature: Enhanced reporting export\n\n  Scenario: Export test run results\n    * def result = karate.toCsv(response)\n    * print result\n"
    return f"# {banner}\n"


def _demo_codegen_response(architecture: "ArchitectureOutput", root: Path, allowed_paths: list[str]) -> str:
    """Deterministic stand-in for the real Codegen LLM conversation
    (Settings.codegen_demo_mode=true). Writes real files into the
    checked-out workspace for every path the real, already-approved
    Architecture artifact named — never a hardcoded/invented file list —
    then returns a CodegenOutput JSON string through the exact same
    downstream parsing/snapshot-diff/scope-check/artifact-persistence
    path real Codegen output goes through. The actual created/modified
    files reported on the resulting artifact still come from
    _diff_snapshot's real filesystem comparison, not from this function's
    own claims. Never invoked unless codegen_demo_mode=True."""
    changes: list[dict] = []
    created_files: list[str] = []
    for task_design in architecture.task_designs:
        for path in task_design.files:
            target = _safe_join(root, path.removeprefix("./"))
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_text(_demo_file_content(path, task_design.approach), encoding="utf-8")
            created_files.append(path)
            changes.append({"path": path, "change_type": "created", "description": f"Demo implementation for: {task_design.approach}"})

    output = {
        "summary": f"[Demo execution] Implemented '{architecture.design_summary}' per the approved architecture ({len(created_files)} file(s)).",
        "modified_files": [],
        "created_files": created_files,
        "deleted_files": [],
        "changes": changes,
        "implementation_notes": (
            "Generated by Quipu's Codegen demo mode (CODEGEN_DEMO_MODE=true) — a deterministic "
            "stand-in for the real Gemini conversation, consuming the real approved Architecture "
            "artifact and writing real files into the checked-out repository workspace."
        ),
        "unresolved_items": [],
        "tests_to_run": list(allowed_paths),
    }
...
class CodegenAgent(QuipuAgent):
    ...
    async def _perform(self, agent_input: AgentInput, context: AgentContext) -> AgentOutput:
        ...
        final_text = ""
        if settings.codegen_demo_mode:
            # Deterministic stand-in for the hackathon demo — no ADK
            # runner, no Gemini call, no with_timeout wait at all. Still
            # writes real files into the real provisioned workspace (see
            # _demo_codegen_response), so the unchanged snapshot-diff/
            # scope-check/artifact-persistence code below behaves
            # identically to a real Codegen run. Never reached unless
            # Settings.codegen_demo_mode=True (default False).
            final_text = _demo_codegen_response(architecture, root, sorted(allowed_paths))
        else:
        ...
        codegen_payload = codegen_output.model_dump(mode="json")
        if settings.codegen_demo_mode:
            # Artifact has no dedicated metadata field (app/domain/artifact.py)
            # — payload is the existing, already-arbitrary extension point,
            # same as Testing's own raw_test_executions key below. Never
            # present unless codegen_demo_mode=True; real-mode payload is
            # byte-identical to before this setting existed.
            codegen_payload["execution_mode"] = "demo"
        artifact = Artifact(
